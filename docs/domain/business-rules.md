...
| BR-E-19 | An Event may carry a `thread_ref`: the conversation the channel says this message belongs to, opaque and scoped by `source_system` exactly as `source_ref` is (ADR-0072). A connector reports what the channel told it and never derives one from content — a subject line is not a thread. It is frozen with the rest of the Event (BR-E-17). Two Events sharing a `thread_ref` is an assertion the channel made, which is what lets BR-AI-39 treat it as evidence rather than as similarity. | INV |
| BR-E-20 | **`processing_status` says what extraction did with this Event, and extraction is the only thing that sets it.** `received` at capture; `extracted` once an analysis completed, whether or not it produced anything — finding nothing in a message is a result, not a failure; `failed` with `processing_error` when the analysis could not complete, so a message that was never read is distinguishable from one that was read and yielded nothing; `skipped` when the Event is not extraction-eligible (BR-E-11, BR-E-08). A repeat analysis overwrites it: the field reports the latest attempt, and the history of attempts is `ai_interaction`. The column and its place in the mutable set (BR-E-17) predate this rule by twenty-three checkpoints and nothing wrote it, so every Event read `received` while the system had in fact analysed all of them. | INV |
...
| BR-AI-39 | **A proposed link comes from the approval chain, never from prose** (ADR-0073). AI may propose `fulfilling_work_id` or `project_id` only by naming a member of the candidate set derived deterministically for the Event being read: this Event's analysis produced a Proposal a person approved which executed into that Work; another Event sharing its `thread_ref` (BR-E-19) did; or that Work belongs to that Project. Each candidate carries a reason code and the ApprovalRecord that produced it. Only Work candidates are offered to the model; a Project is derived from the Work item it named and is never chosen by it, which is what leaves BR-AI-35 untouched. An identifier outside the set is **refused** by the intent validator, not discouraged by a prompt. An empty candidate set leaves the field empty with the reason recorded (BR-AI-17), which is a correct Proposal and not an incomplete one. Nothing here changes the autonomy level: the link is proposed and approved like the rest of the action. | INV |
